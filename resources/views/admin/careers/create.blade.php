    ...
    <form action="{{ route('admin.careers.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="careername" class="form-control"
                   ...
                   placeholder="e.g. Machine Operator / Technician" required>
        </div>

        <div class="mb-3">
            <label class="form-label">Description <span style="color:#c0281c;">*</span></label>
            <textarea name="careerdesc" class="form-control" rows="3"
                      placeholder="Brief description of the role..." required>{{ old('careerdesc') }}</textarea>
        </div>

        <div class="mb-3">
            <label class="form-label">Requirements <span style="color:#aaa; font-weight:400;">(optional)</span></label>
            <textarea name="careerrspec" class="form-control" rows="3"
                      placeholder="e.g. Min. D3, 2 years experience...">{{ old('careerrspec') }}</textarea>
        </div>

        <div class="mb-3">
            <label class="form-label">Benefit <span style="color:#aaa; font-weight:400;">(optional)</span></label>
            <input type="number" name="careerbenefit" class="form-control"
                   value="{{ old('careerbenefit') }}"
